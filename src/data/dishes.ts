...
export const DISHES: Dish[] = [
  ...
  d({
    ...
    pins: [
      { x: 55, y: 21, label: 'Brioche crown', kind: 'carb', conf: 96 },
      { x: 62, y: 44, label: 'Beefsteak tomato', kind: 'produce', conf: 93 },
      { x: 30, y: 33, label: 'Iceberg, shaved', kind: 'produce', conf: 90 },
      { x: 45, y: 55, label: 'Double smashed patty', kind: 'protein', conf: 92, note: 'cheddar between' },
      { x: 65, y: 87, label: "Bird's-eye chili", kind: 'garnish', conf: 85 },
    ],
    ingredients: [
  ...
  d({
    ...
    pins: [
      { x: 45, y: 32, label: 'Spaghetti, al dente', kind: 'carb', conf: 95 },
      { x: 49, y: 11, label: 'Baby spinach', kind: 'produce', conf: 89 },
      { x: 16, y: 70, label: 'Cherry tomatoes, burst', kind: 'produce', conf: 92 },
      { x: 21, y: 45, label: 'Fresh mozzarella', kind: 'dairy', conf: 84, note: 'or burrata' },
      { x: 58, y: 60, label: 'Cracked pepper & oil', kind: 'sauce', conf: 80 },
    ],
    ingredients: [
  ...
  d({
    ...
    pins: [
      { x: 62, y: 45, label: 'Marinated ribeye', kind: 'protein', conf: 82, note: 'possibly brisket cut' },
      { x: 50, y: 21, label: 'Scallion & sesame', kind: 'garnish', conf: 90 },
      { x: 92, y: 80, label: 'Kimchi, aged', kind: 'produce', conf: 76 },
      { x: 13, y: 86, label: 'Pickled greens', kind: 'produce', conf: 71 },
  ...
  d({
    ...
    pins: [
      { x: 48, y: 54, label: 'Guacamole', kind: 'sauce', conf: 88 },
      { x: 30, y: 62, label: 'Chicken al pastor', kind: 'protein', conf: 72, note: 'pork traditional' },
      { x: 64, y: 71, label: 'Shaved cabbage', kind: 'produce', conf: 81 },
      { x: 25, y: 79, label: 'Corn tortilla ×2', kind: 'carb', conf: 78 },
      { x: 13, y: 45, label: 'Lime, fresh cut', kind: 'garnish', conf: 93 },
    ],
    ingredients: [
  ...
  d({
    ...
    pins: [
      { x: 38, y: 68, label: 'Glazed chicken thigh', kind: 'protein', conf: 92 },
      { x: 66, y: 27, label: 'Carrot ribbon slaw', kind: 'produce', conf: 88 },
      { x: 38, y: 36, label: 'Baby greens', kind: 'produce', conf: 85 },
      { x: 83, y: 57, label: 'Lemon wedge', kind: 'garnish', conf: 87 },
      { x: 50, y: 80, label: 'Toasted sesame', kind: 'garnish', conf: 94 },
    ],
    ingredients: [
  ...
  d({
    ...
    pins: [
      { x: 48, y: 50, label: 'Fresh lo mein noodles', kind: 'carb', conf: 87 },
      { x: 16, y: 62, label: 'Bamboo chopsticks', kind: 'garnish', conf: 96 },
      { x: 80, y: 30, label: 'Red pepper, flash-fried', kind: 'produce', conf: 84 },
      { x: 87, y: 17, label: 'Broccoli floret', kind: 'produce', conf: 80 },
      { x: 45, y: 80, label: 'Soy-oyster gloss', kind: 'sauce', conf: 82 },
    ],
    ingredients: [
  ...
  d({
    ...
    pins: [
      { x: 45, y: 38, label: 'Feta, barrel-aged', kind: 'dairy', conf: 91 },
      { x: 68, y: 46, label: 'Cherry tomatoes', kind: 'produce', conf: 94 },
      { x: 43, y: 60, label: 'Kalamata olives', kind: 'produce', conf: 80 },
      { x: 33, y: 29, label: 'Frisée & cucumber', kind: 'produce', conf: 83 },
      { x: 56, y: 72, label: 'Oregano oil', kind: 'sauce', conf: 74 },
    ],
    ingredients: [
  ...
  d({
    ...
    story: 'Ninety seconds in front of live flame.', photoKey: 'pizza-fire', focus: [50, 58], conf: 'medium', confPct: 77,
    kcal: [540, 640], protein: [20, 26], carbs: [64, 74], fat: [20, 27], servings: 2, minutes: 90,
    tags: ['wood-fire', 'weekend'], pins: [
      { x: 12, y: 79, label: 'Leopard-spotted crust', kind: 'carb', conf: 72 },
      { x: 15, y: 17, label: 'Live oak fire', kind: 'garnish', conf: 98 },
      { x: 55, y: 40, label: 'Buffalo mozzarella', kind: 'dairy', conf: 74 },
      { x: 34, y: 67, label: 'Strawberries & spinach', kind: 'produce', conf: 89 },
      { x: 84, y: 55, label: 'Pepperoni rounds', kind: 'protein', conf: 86 },
    ],
  ...
  d({
    ...
    story: 'A broth that took its time, noodles that didn’t.', photoKey: 'noodle-soup', focus: [50, 50], conf: 'medium', confPct: 72,
    kcal: [390, 470], protein: [22, 28], carbs: [52, 60], fat: [8, 13], servings: 2, minutes: 40,
    tags: ['broth', 'fresh-herbs'], pins: [
      { x: 30, y: 62, label: 'Rice noodles', kind: 'carb', conf: 88 },
      { x: 42, y: 30, label: 'Poached shrimp', kind: 'protein', conf: 88 },
      { x: 80, y: 36, label: 'Mint & chili', kind: 'garnish', conf: 90 },
      { x: 56, y: 79, label: 'Quail eggs', kind: 'protein', conf: 86 },
    ],
  ...
  d({
    ...
    story: 'Butter-griddled rolls and a curry that eats like a hug.', photoKey: 'pav-bhaji', focus: [50, 52], conf: 'medium', confPct: 76,
    kcal: [470, 560], protein: [12, 16], carbs: [58, 68], fat: [20, 26], servings: 4, minutes: 45,
    tags: ['street-food', 'vegetarian'], pins: [
      { x: 25, y: 70, label: 'Mashed vegetable bhaji', kind: 'produce', conf: 84 },
      { x: 75, y: 32, label: 'Pav rolls, buttered', kind: 'carb', conf: 92 },
      { x: 82, y: 67, label: 'Red onion, chopped', kind: 'garnish', conf: 89 },
      { x: 9, y: 49, label: 'Lime wedges', kind: 'garnish', conf: 90 },
      { x: 39, y: 60, label: 'Butter, melting', kind: 'dairy', conf: 82 },
    ],
  ...
  d({
    ...
    story: 'Sunday morning, stacked seven high.', photoKey: 'pancakes', focus: [48, 50], conf: 'high', confPct: 92,
    kcal: [610, 720], protein: [12, 16], carbs: [88, 98], fat: [22, 28], servings: 2, minutes: 25,
    tags: ['sweet', 'weekend'], pins: [
      { x: 45, y: 58, label: 'Buttermilk stack ×7', kind: 'carb', conf: 84 },
      { x: 50, y: 25, label: 'Strawberries & blueberries', kind: 'produce', conf: 89 },
      { x: 60, y: 47, label: 'Salted caramel', kind: 'sauce', conf: 87 },
      { x: 82, y: 36, label: 'Chantilly', kind: 'dairy', conf: 81 },
    ],
  ...
  d({
    ...
    story: 'Cucumber-cool, rolled tight, cut clean.', photoKey: 'sushi', focus: [45, 50], conf: 'high', confPct: 88,
    kcal: [310, 380], protein: [7, 10], carbs: [58, 66], fat: [4, 7], servings: 2, minutes: 40,
    tags: ['no-cook', 'light'], pins: [
      { x: 48, y: 50, label: 'Cucumber-avocado maki', kind: 'produce', conf: 90 },
      { x: 88, y: 32, label: 'Chopsticks', kind: 'garnish', conf: 97 },
      { x: 76, y: 17, label: 'Carrot rose', kind: 'garnish', conf: 86 },
    ],
  ...
  d({
    ...
    tags: ['vegan', 'meal-prep'], pins: [
      { x: 74, y: 78, label: 'Charred brussels & squash', kind: 'produce', conf: 89 },
      { x: 66, y: 42, label: 'Herbed quinoa', kind: 'carb', conf: 85 },
      { x: 13, y: 12, label: 'Zucchini, market-fresh', kind: 'produce', conf: 88 },
      { x: 17, y: 61, label: 'Lime & tomato mise', kind: 'garnish', conf: 91 },
      { x: 90, y: 30, label: 'Ginger root', kind: 'garnish', conf: 90 },
    ],
